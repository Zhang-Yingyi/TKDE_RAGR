...
def parse_args():
    ...
    parser.add_argument("--data_path", type=str,default="Instruments", help='dataset')
...
# output_file = f"{dataset}.review.json"
output_file = f"{dataset}.review.json"
...
inter_path = os.path.join(output_dir, f'{dataset}.inter.json')
rev_inter = load_json(inter_path)
rev_ls = []
for item in rev_inter:
    data = rev_inter[item]
    rev_data = data[1]
    rev_ls+=rev_data

all_indices_dict = {}
for item, indices in zip(rev_ls, all_indices.tolist()):
    if item not in all_indices_dict:
