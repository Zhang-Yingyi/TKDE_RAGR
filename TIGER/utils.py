...
from data import SeqRecDataset, SeqRevRecDataset, SeqRevJointRecDataset, SeqRevRecDatasetPost, SeqRevRecDataset_tiger
...
def parse_dataset_args(parser):
    ...
    parser.add_argument("--rev_file", type=str, default=".review.json", help="the item indices file")
...
def parse_train_args(parser):
    ...
    parser.add_argument("--sft_ckpt", type=str, default='./ckpt_rev/Beauty')
...
def parse_test_args(parser):
    ...
    parser.add_argument("--pair_mode",type=str, default="train")
...
def load_rev_test_dataset(args):

    if args.test_task.lower() == "seqrec":
        # test_data = SeqRecDataset(args, mode="test_ranking", sample_num=args.sample_num)
        test_data = SeqRevRecDataset(args, mode="test", sample_num=args.sample_num)
    else:
        raise NotImplementedError

    return test_data

def load_rev_test_dataset_pair(args):

    if args.test_task.lower() == "seqrec":
        # test_data = SeqRecDataset(args, mode="test_ranking", sample_num=args.sample_num)
        test_data = SeqRevRecDataset(args, mode="train", sample_num=args.sample_num)
    else:
        raise NotImplementedError
...
def load_rev_post_datasets(args):
    ...
    valid_data = SeqRevRecDatasetPost(args,"valid",2)
...
def load_rev_post_test_dataset(args):
    ...
    if args.test_task.lower() == "seqrec":
        # test_data = SeqRecDataset(args, mode="test_ranking", sample_num=args.sample_num)
        test_data = SeqRevRecDatasetPost(args, mode="test", sample_num=args.sample_num)
    else:
        raise NotImplementedError
...
def load_rev_datasets_joint(args):
    ...
    for task, prompt_sample_num,data_sample_num in zip(tasks,train_prompt_sample_num,train_data_sample_num):
        if task.lower() == "seqrec":
            dataset = SeqRevJointRecDataset(args, mode="train", prompt_sample_num=prompt_sample_num, sample_num=data_sample_num)
        else:
            raise NotImplementedError
        train_datasets.append(dataset)
    ...
    valid_data = SeqRevJointRecDataset(args,"valid",2)

    return train_data, valid_data

def load_rev_test_dataset_joint(args):

    if args.test_task.lower() == "seqrec":
        # test_data = SeqRecDataset(args, mode="test_ranking", sample_num=args.sample_num)
        test_data = SeqRevJointRecDataset(args, mode="test", sample_num=args.sample_num)
    else:
        raise NotImplementedError
...
def prefix_allowed_tokens_fn_qwen(candidate_trie, prompt_length, eos_token_id):
    def prefix_allowed_tokens(batch_id, sentence):
        # 1. 把 1D Tensor 转成 list，并只截取模型新生成的部分
        # sentence 是包含了 [Prompt + Generated] 的完整序列
        generated_ids = sentence[prompt_length:].tolist()
        
        # 2. 在前面拼上 [0]，伪装成 T5 的格式，去适配你原来的 Trie 树
        search_path = [0] + generated_ids
        
        # 3. 去 Trie 树里查询
        trie_out = candidate_trie.get(search_path)
        
        # 4. 兜底保护：如果 Trie 树没找到合法的下一步，强制返回 EOS 结束生成
        if not trie_out:
            return [eos_token_id]
